			<form id="signin_teacher" class="form-signin" method="post">
					...
					<button id="signin" name="login" class="btn btn-success" type="submit"><i class="icon-check icon-large"></i> Sign in</button>
			</form>
			<script>
			...
			jQuery("#signin_teacher").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Hello Sir", { header: 'Sign up Success' });
						var delay = 1000;
							setTimeout(function(){ window.location = 'dasboard_teacher.php'  }, delay);  
						}else{
							$.jGrowl("Sir your data was not found in the database", { header: 'Sign Up Failed' });
						}
						}
					});
